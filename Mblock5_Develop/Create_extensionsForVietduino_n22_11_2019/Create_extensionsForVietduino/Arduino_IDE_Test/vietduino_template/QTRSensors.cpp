...
void QTRSensors::setTimeout(uint16_t timeout)
{
  ...
  _timeout = timeout;
  if (_type == QTRType_RC) { _maxValue = timeout; }
}
...
// emitters defaults to QTREmitters_All; wait defaults to true
void QTRSensors::emittersOff(QTREmitters emitters, bool wait)
{
  ...
  if (emitters == QTREmitters_All ||
      (_emitterPinCount == 2 && emitters == QTREmitters_Odd))
  {
  ...
  if (_emitterPinCount == 2 &&
      (emitters == QTREmitters_All || emitters == QTREmitters_Even))
  {
    // Check if pin is defined and only turn off if not already off
    if ((_evenEmitterPin != QTRNoEmitterPin) &&
...
void QTRSensors::emittersSelect(QTREmitters emitters)
{
  ...
  switch (emitters)
  {
    case QTREmitters_Odd:
      offEmitters = QTREmitters_Even;
      break;

    case QTREmitters_Even:
      offEmitters = QTREmitters_Odd;
      break;

    case QTREmitters_All:
      emittersOn();
      return;

    case QTREmitters_None:
      emittersOff();
      return;
...
void QTRSensors::calibrate(QTRReadMode mode)
{
  ...
  if (mode == QTRReadMode_Manual) { return; }

  if (mode == QTRReadMode_On ||
      mode == QTRReadMode_OnAndOff)
  {
    calibrateOnOrOff(calibrationOn, QTRReadMode_On);
  }
  else if (mode == QTRReadMode_OddEven ||
           mode == QTRReadMode_OddEvenAndOff)
  {
    calibrateOnOrOff(calibrationOn, QTRReadMode_OddEven);
  }

  if (mode == QTRReadMode_OnAndOff ||
      mode == QTRReadMode_OddEvenAndOff ||
      mode == QTRReadMode_Off)
  {
    calibrateOnOrOff(calibrationOff, QTRReadMode_Off);
  }
}
...
void QTRSensors::read(uint16_t * sensorValues, QTRReadMode mode)
{
  switch (mode)
  {
    case QTRReadMode_Off:
      emittersOff();
      // fall through
    case QTRReadMode_Manual:
      readPrivate(sensorValues);
      return;

    case QTRReadMode_On:
    case QTRReadMode_OnAndOff:
      emittersOn();
      readPrivate(sensorValues);
      emittersOff();
      break;

    case QTRReadMode_OddEven:
    case QTRReadMode_OddEvenAndOff:
      // Turn on odd emitters and read the odd-numbered sensors.
      // (readPrivate takes a 0-based array index, so start = 0 to start with
      // the first sensor)
      emittersSelect(QTREmitters_Odd);
      ...
      // the second sensor)
      emittersSelect(QTREmitters_Even);
...
void QTRSensors::readPrivate(uint16_t * sensorValues, uint8_t start, uint8_t step)
{
  ...
  switch (_type)
  {
    ...
    default: // QTRType_Undefined or invalid - do nothing
      return;
  }
}
                         ...
                         bool invertReadings)
{
  ...
  // manual emitter control is not supported
  if (mode == QTRReadMode_Manual) { return 0; }
